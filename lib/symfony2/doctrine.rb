  ...
  namespace :doctrine do
    namespace :cache do
      ...
      task :clear_metadata, :roles => :app, :except => { :no_release => true } do
        ...
        run "sh -c 'cd #{latest_release} && #{php_bin} #{symfony_console} doctrine:cache:clear-metadata --env=#{symfony_env_prod}'"
        puts_ok
      end
      ...
      task :clear_query, :roles => :app, :except => { :no_release => true } do
        ...
        run "sh -c 'cd #{latest_release} && #{php_bin} #{symfony_console} doctrine:cache:clear-query --env=#{symfony_env_prod}'"
        puts_ok
      end
      ...
      task :clear_result, :roles => :app, :except => { :no_release => true } do
        ...
        run "sh -c 'cd #{latest_release} && #{php_bin} #{symfony_console} doctrine:cache:clear-result --env=#{symfony_env_prod}'"
        puts_ok
      end
    end
      ...
      [:create, :update, :drop].each do |action|
        namespace :schema do
          ...
          task action, :roles => :app, :except => { :no_release => true } do
            ...
            run "sh -c 'cd #{latest_release} && #{php_bin} #{symfony_console} doctrine:mongodb:schema:#{action.to_s} --env=#{symfony_env_prod}'", :once => true
            puts_ok
          end
        end
        ...
        if action != :update
          namespace :indexes do
            ...
            task action, :roles => :app do
              ...
              run "sh -c 'cd #{latest_release} && #{php_bin} #{symfony_console} doctrine:mongodb:schema:#{action.to_s} --index --env=#{symfony_env_prod}'", :once => true
              puts_ok
            end
          end
        end
    ...
    namespace :init do
      ...
      task :acl, :roles => :app, :except => { :no_release => true } do
        ...
        run "sh -c 'cd #{latest_release} && #{php_bin} #{symfony_console} init:acl --env=#{symfony_env_prod}'", :once => true
        puts_ok
      end
    end
  end
